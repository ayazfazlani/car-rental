...
export default function Page({ params: paramsPromise }: { params: Promise<TParams> }) {
    const params = use(paramsPromise)
    const router = useRouter()
    const [blogToEdit, setBlogToEdit] = React.useState<Blog>()
    const [tag, setTag] = React.useState<string>('')
    const [keyword, setKeyword] = React.useState<string>('')
    const [content, setContent] = React.useState<Content>({})

    const { mutate: createBlog, isPending } = useMutation({
        mutationFn: (data: TCreateBlog) => API.queryPut<Blog>({ url: '/api/admin/blog/' + blogToEdit!.id, payload: data, auth: true }),
        onSuccess: (data: Blog) => {
    ...
    const getBlog = async () => {
        const { data, success, message } = await API.get<Blog>({ url: `/api/admin/blog/${params.slug}`, auth: true })
        if (!success) {
            toast.error(message)
        }
        if (success && data) {
        ...
        <div className='flex flex-col min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-4 sm:p-8'>
                                ...
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem>
                                            ...
                                            <p className='text-xs text-slate-500 mt-3 text-center'>Recommended size: 1200x630px</p>
                                        </FormItem>
                                    )}
                                />
                            </CardContent>
            ...
            <div className='mt-8'>
                <Card className='border-slate-200 shadow-md hover:shadow-lg transition-shadow'>
                    ...
                    <CardContent className='pt-6'>
                        {blogToEdit && <AddContent content={content} updateContent={updateContent} />}
                    </CardContent>
                </Card>
            </div>
        </div>
